...
const getUserForBinance = async (userId, username) => {
    if (userId !== undefined && userId !== null && String(userId).trim() !== '') {
        const parsedId = parseInt(String(userId), 10);
        if (!Number.isNaN(parsedId)) {
            return getUserByIdWithQr(parsedId);
        }
    }
    if (username && String(username).trim() !== '') {
        return getUserByUsernameWithQr(username);
    }
    return null;
};

const parseApiKeysFromUser = (user) => {
    ...
    const apiKey = String(
        keyData.apiKey || keyData.api_key || keyData.apikey || ''
    ).trim();
    const secretKey = String(
        keyData.secretKey || keyData.secret_key || keyData.secretkey || ''
    ).trim();
...
const getPortfolio = async (req, res) => {
    ...
    try {
        const user = await getUserForBinance(user_id, username);
        ...
        let secretKey;
        try {
            const keys = parseApiKeysFromUser(user);
            apiKey = keys.apiKey;
        ...
        return res.status(500).json({ error: 'Gagal mengambil data portofolio dari Binance', detail: err.message });
    }
};
...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        return res.json({ message: 'Order berhasil dieksekusi', data: orderResponse });
    } catch (err) {
        let msg = err.message;
        ...
        else if (msg.includes('Invalid API-key, IP, or permissions')) {
            msg = 'API Key salah, kadaluarsa, atau tidak memiliki izin Spot Trading.';
        }

        return res.status(400).json({ error: 'Gagal mengeksekusi order di Binance', detail: msg, original: err.message });
    }
};
...
const getKlines = async (req, res) => {
    ...
    try {
        ...
        return res.json({ source: 'binance', symbol, interval, limit, count: klines.length, data: klines });
    } catch (err) {
        return res.status(500).json({ message: 'Gagal mengambil data kline.', error: err.message });
    }
};

const getKlinesBatch = async (req, res) => {
    ...
    try {
        ...
        return res.json({ source: 'binance', interval, limit, data: results });
    } catch (err) {
        return res.status(500).json({ message: 'Gagal mengambil data kline batch.', error: err.message });
    }
};

const sseClients = new Set();

const getPricesStream = (req, res) => {
    ...
    const sendData = (payload) => {
        if (!priceCache.updatedAt) return;
...
setInterval(() => {
    if (sseClients.size === 0 || !priceCache.updatedAt) return;
